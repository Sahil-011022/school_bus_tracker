...
function ParentInterface() {
  ...
  useEffect(() => {
    if (!busId) return;
    const q = query(collection(db, 'buses', busId, 'locations'), orderBy('timestamp', 'desc'), limit(1));
  ...
  return (
    <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100">
      <h2 className="text-2xl font-bold mb-6 tracking-tight">Parent View</h2>
      <input
        type="text"
      ...
      />
      <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl h-96">
        {location && (
          <APIProvider apiKey={process.env.GOOGLE_MAPS_API_KEY || ''}>
            <Map defaultCenter={location} defaultZoom={13} center={location}>
